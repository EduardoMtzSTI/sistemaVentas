		...
		<div id="list-producto" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="precioActual" title="${message(code: 'producto.precioActual.label', default: 'Precio Actual')}" />
					
						<th><g:message code="producto.stock.label" default="Stock" /></th>
				...
				<g:each in="${productoInstanceList}" status="i" var="productoInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td>${fieldValue(bean: productoInstance, field: "precioActual")}</td>
					
						<td>${fieldValue(bean: productoInstance, field: "stock")}</td>
